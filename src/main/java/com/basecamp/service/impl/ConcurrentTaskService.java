...
public class ConcurrentTaskService {
    ...
    public void handleBugRace(int threadCount){
        ExecutorService bugExecutor =
        ...
        Runnable run = () -> {
            ...
            while (bugCount < 100) {
                ...
                bugCount += Math.random() * 20;
                if (bugCount < 100) {
                    log.info(Thread.currentThread().getName() + " - " + bugCount);
                }
            }
            if (bugCount >= 100) {
                log.info(Thread.currentThread().getName() + " is Finish");
                bugExecutor.shutdown();
            }
        };
        for(int i = 0 ; i < threadCount ; i++)
            bugExecutor.execute(run);
    }
}
